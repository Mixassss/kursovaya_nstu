...
#include "ui_lec3_test3.h"
#include "lec3_test4.h"
#include <QRandomGenerator>
#include <QRadioButton>
#include <QMessageBox>

lec3_test3::lec3_test3(QWidget *parent)
    : QDialog(parent)
    , ui(new Ui::lec3_test3)
    , currentPair1(-1)
    , currentPair2(-1)
{
    ui->setupUi(this);
    ...
    // Подключаем сигналы
    connect(ui->agree, &QCheckBox::stateChanged, this, &lec3_test3::onAgreeClicked);
    connect(ui->pushButton, &QPushButton::clicked, this, &lec3_test3::onAnswerClicked);
    connect(ui->retry, &QCheckBox::stateChanged, this, &lec3_test3::onRetryClicked);
    connect(ui->pushButton_2, &QPushButton::clicked, this, &lec3_test3::onNextLectureClicked);
}

void lec3_test3::onRetryClicked(int state) {
    if (state == Qt::Checked) {
        resetTest();
    }
}

void lec3_test3::resetTest() {
...
void lec3_test3::resetRadioButtons(QGroupBox* group) {
...
void lec3_test3::initQuestionPairs() {
...
void lec3_test3::hideAllGroupElements(QGroupBox* group) {
    foreach(QObject* child, group->children()) {
...
void lec3_test3::onAgreeClicked(int state) {
    if (state == Qt::Checked) {
        startNewTest();
    }
}

void lec3_test3::startNewTest() {
    ui->scrollArea->hide();
...
void lec3_test3::showRandomQuestionPair(QGroupBox* group, const QVector<QuestionPair>& pairs) {
...
void lec3_test3::connectRadioButtons(QGroupBox* group) {
    foreach(QObject* child, group->children()) {
        if (auto radio = qobject_cast<QRadioButton*>(child)) {
            connect(radio, &QRadioButton::toggled, this, &lec3_test3::checkAnswers, Qt::UniqueConnection);
        }
    }
}

void lec3_test3::checkAnswers() {
...
bool lec3_test3::isGroupAnswered(QGroupBox* group) {
    foreach(QObject* child, group->children()) {
        if (auto radio = qobject_cast<QRadioButton*>(child)) {
            if (radio->isChecked()) {
                return true;
...
void lec3_test3::onAnswerClicked() {
    // Проверяем ответы
...
bool lec3_test3::checkGroupAnswer(QGroupBox* group, const QVector<QuestionPair>& pairs, int pairIndex) {
...
void lec3_test3::setRadioButtonsEnabled(QGroupBox* group, bool enabled) {
    foreach(QObject* child, group->children()) {
        if (auto radio = qobject_cast<QRadioButton*>(child)) {
...
void lec3_test3::onNextLectureClicked() {
    // Создаем новое окно lec3_test2
    lec3_test4 *test4Window = new lec3_test4();
