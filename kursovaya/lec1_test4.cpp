...
#include "test1.h"
#include "ui_lec1_test4.h"
#include <QRandomGenerator>
#include <QRadioButton>
#include <QMessageBox>

lec1_test4::lec1_test4(QWidget *parent)
    : QDialog(parent)
    , ui(new Ui::lec1_test4)
    , currentPair1(-1)
    , currentPair2(-1)
{
    ui->setupUi(this);
    ...
    // Подключаем сигналы
    connect(ui->agree, &QCheckBox::stateChanged, this, &lec1_test4::onAgreeClicked);
    connect(ui->pushButton, &QPushButton::clicked, this, &lec1_test4::onAnswerClicked);
    connect(ui->retry, &QCheckBox::stateChanged, this, &lec1_test4::onRetryClicked);
    connect(ui->pushButton_2, &QPushButton::clicked, this, &lec1_test4::onNextLectureClicked);
}

void lec1_test4::onRetryClicked(int state) {
    if (state == Qt::Checked) {
        resetTest();
    }
}

void lec1_test4::resetTest() {
...
void lec1_test4::resetRadioButtons(QGroupBox* group) {
...
void lec1_test4::initQuestionPairs() {
    // Группа 1 пары (вопрос-ответ-пояснение-правильный ответ)
    group1Pairs = {
        {ui->first_q, ui->first_ans, ui->first_poyas, ui->Answer_A},
        {ui->third_q, ui->third_ans, ui->third_poyas, ui->Answer_C},
        {ui->fifth_q, ui->fifth_ans, ui->five_poyas, ui->Answer_C},
    ...
    group2Pairs = {
        ...
        {ui->eighth_q, ui->eighth_ans, ui->eight_poyas, ui->Answer_C_2}
    };
}

void lec1_test4::hideAllGroupElements(QGroupBox* group) {
    foreach(QObject* child, group->children()) {
...
void lec1_test4::onAgreeClicked(int state) {
    if (state == Qt::Checked) {
        startNewTest();
    }
}

void lec1_test4::startNewTest() {
    ui->scrollArea->hide();
...
void lec1_test4::showRandomQuestionPair(QGroupBox* group, const QVector<QuestionPair>& pairs) {
...
void lec1_test4::connectRadioButtons(QGroupBox* group) {
    foreach(QObject* child, group->children()) {
        if (auto radio = qobject_cast<QRadioButton*>(child)) {
            connect(radio, &QRadioButton::toggled, this, &lec1_test4::checkAnswers, Qt::UniqueConnection);
        }
    }
}

void lec1_test4::checkAnswers() {
...
bool lec1_test4::isGroupAnswered(QGroupBox* group) {
    foreach(QObject* child, group->children()) {
        if (auto radio = qobject_cast<QRadioButton*>(child)) {
            if (radio->isChecked()) {
                return true;
...
void lec1_test4::onAnswerClicked() {
    // Проверяем ответы
...
bool lec1_test4::checkGroupAnswer(QGroupBox* group, const QVector<QuestionPair>& pairs, int pairIndex) {
...
void lec1_test4::setRadioButtonsEnabled(QGroupBox* group, bool enabled) {
    foreach(QObject* child, group->children()) {
        if (auto radio = qobject_cast<QRadioButton*>(child)) {
...
void lec1_test4::onNextLectureClicked() {
    // Закрываем текущее окно (lec1_test4)
    this->close();

    // Находим и закрываем окно lec1_test1
    QWidgetList widgets = QApplication::topLevelWidgets();
    for (QWidget* widget : widgets) {
        if (widget->inherits("test1")) {
            widget->close();
            break;
        }
    }

    // Можно показать сообщение о переходе (опционально)
    QMessageBox::information(nullptr, "Переход", "Окна тестов закрыты. Переход к следующему этапу...");
}

lec1_test4::~lec1_test4() {
    delete ui;
}
