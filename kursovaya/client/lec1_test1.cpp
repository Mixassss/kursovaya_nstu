...
#include "ui_lec1_test1.h"
#include <QRandomGenerator>
#include <QRadioButton>
#include <QMessageBox>

lec1_test1::lec1_test1(QWidget *parent)
    : QDialog(parent)
    , ui(new Ui::lec1_test1)
    , currentPair1(-1)
    , currentPair2(-1)
{
    ui->setupUi(this);
    ...
    // Подключаем сигналы
    connect(ui->agree, &QCheckBox::stateChanged, this, &lec1_test1::onAgreeClicked);
    connect(ui->pushButton, &QPushButton::clicked, this, &lec1_test1::onAnswerClicked);
    connect(ui->retry, &QCheckBox::stateChanged, this, &lec1_test1::onRetryClicked);
    connect(ui->pushButton_2, &QPushButton::clicked, this, &lec1_test1::onNextLectureClicked);
}

void lec1_test1::onRetryClicked(int state) {
    if (state == Qt::Checked) {
        resetTest();
    }
}

void lec1_test1::resetTest() {
...
void lec1_test1::resetRadioButtons(QGroupBox* group) {
...
void lec1_test1::initQuestionPairs() {
    // Группа 1 пары (вопрос-ответ-пояснение-правильный ответ)
    group1Pairs = {
        {ui->first_q, ui->first_ans, ui->first_poyas, ui->Answer_A},
        {ui->third_q, ui->third_ans, ui->third_poyas, ui->Answer_B},
        {ui->fifth_q, ui->fifth_ans, ui->five_poyas, ui->Answer_B},
        {ui->seven_q, ui->seven_ans, ui->seven_poyas, ui->Answer_A},
        {ui->ninth_q, ui->ninth_ans, ui->ninth_poyas, ui->Answer_D}
    };

    // Группа 2 пары
    group2Pairs = {
        {ui->second_q, ui->second_ans, ui->sec_poyas, ui->Answer_B_2},
        {ui->fourth_q, ui->fourth_ans, ui->fourth_poyas, ui->Answer_A_2},
        {ui->six_q, ui->six_ans, ui->six_poyas, ui->Answer_D_2},
        {ui->eighth_q, ui->eighth_ans, ui->eight_poyas, ui->Answer_B_2}
    };
}

void lec1_test1::hideAllGroupElements(QGroupBox* group) {
    foreach(QObject* child, group->children()) {
...
void lec1_test1::onAgreeClicked(int state) {
    if (state == Qt::Checked) {
        startNewTest();
    }
}

void lec1_test1::startNewTest() {
    ui->scrollArea->hide();
...
void lec1_test1::showRandomQuestionPair(QGroupBox* group, const QVector<QuestionPair>& pairs) {
...
void lec1_test1::connectRadioButtons(QGroupBox* group) {
    foreach(QObject* child, group->children()) {
        if (auto radio = qobject_cast<QRadioButton*>(child)) {
            connect(radio, &QRadioButton::toggled, this, &lec1_test1::checkAnswers, Qt::UniqueConnection);
        }
    }
}

void lec1_test1::checkAnswers() {
...
bool lec1_test1::isGroupAnswered(QGroupBox* group) {
    foreach(QObject* child, group->children()) {
        if (auto radio = qobject_cast<QRadioButton*>(child)) {
            if (radio->isChecked()) {
                return true;
...
void lec1_test1::onAnswerClicked() {
    // Проверяем ответы
...
bool lec1_test1::checkGroupAnswer(QGroupBox* group, const QVector<QuestionPair>& pairs, int pairIndex) {
...
void lec1_test1::setRadioButtonsEnabled(QGroupBox* group, bool enabled) {
    foreach(QObject* child, group->children()) {
        if (auto radio = qobject_cast<QRadioButton*>(child)) {
...
void lec1_test1::onNextLectureClicked() {
    // Создаем новое окно lec1_test2
    lec1_test2 *test2Window = new lec1_test2();

    // Настраиваем новое окно
    test2Window->setModal(true); // Если нужно модальное окно
