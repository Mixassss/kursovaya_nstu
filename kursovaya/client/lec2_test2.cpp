...
#include "ui_lec2_test2.h"
#include "lec2_test3.h"
#include <QRandomGenerator>
#include <QRadioButton>
#include <QMessageBox>

lec2_test2::lec2_test2(QWidget *parent)
    : QDialog(parent)
    , ui(new Ui::lec2_test2)
    , currentPair1(-1)
    , currentPair2(-1)
{
    ui->setupUi(this);
    ...
    // Подключаем сигналы
    connect(ui->agree, &QCheckBox::stateChanged, this, &lec2_test2::onAgreeClicked);
    connect(ui->pushButton, &QPushButton::clicked, this, &lec2_test2::onAnswerClicked);
    connect(ui->retry, &QCheckBox::stateChanged, this, &lec2_test2::onRetryClicked);
    connect(ui->pushButton_2, &QPushButton::clicked, this, &lec2_test2::onNextLectureClicked);
}

void lec2_test2::onRetryClicked(int state) {
    if (state == Qt::Checked) {
        resetTest();
    }
}

void lec2_test2::resetTest() {
...
void lec2_test2::resetRadioButtons(QGroupBox* group) {
...
void lec2_test2::initQuestionPairs() {
    // Группа 1 пары (вопрос-ответ-пояснение-правильный ответ)
    group1Pairs = {
        {ui->first_q, ui->first_ans, ui->first_poyas, ui->Answer_D},
        {ui->third_q, ui->third_ans, ui->third_poyas, ui->Answer_B},
        {ui->fifth_q, ui->fifth_ans, ui->five_poyas, ui->Answer_D},
        {ui->seven_q, ui->seven_ans, ui->seven_poyas, ui->Answer_C},
        {ui->ninth_q, ui->ninth_ans, ui->ninth_poyas, ui->Answer_A}
    };

    // Группа 2 пары
    group2Pairs = {
        {ui->second_q, ui->second_ans, ui->sec_poyas, ui->Answer_B_2},
        {ui->fourth_q, ui->fourth_ans, ui->fourth_poyas, ui->Answer_D_2},
        {ui->six_q, ui->six_ans, ui->six_poyas, ui->Answer_D_2},
        {ui->eighth_q, ui->eighth_ans, ui->eight_poyas, ui->Answer_B_2}
    };
}

void lec2_test2::hideAllGroupElements(QGroupBox* group) {
    foreach(QObject* child, group->children()) {
...
void lec2_test2::onAgreeClicked(int state) {
    if (state == Qt::Checked) {
        startNewTest();
    }
}

void lec2_test2::startNewTest() {
    ui->scrollArea->hide();
...
void lec2_test2::showRandomQuestionPair(QGroupBox* group, const QVector<QuestionPair>& pairs) {
...
void lec2_test2::connectRadioButtons(QGroupBox* group) {
    foreach(QObject* child, group->children()) {
        if (auto radio = qobject_cast<QRadioButton*>(child)) {
            connect(radio, &QRadioButton::toggled, this, &lec2_test2::checkAnswers, Qt::UniqueConnection);
        }
    }
}

void lec2_test2::checkAnswers() {
...
bool lec2_test2::isGroupAnswered(QGroupBox* group) {
    foreach(QObject* child, group->children()) {
        if (auto radio = qobject_cast<QRadioButton*>(child)) {
            if (radio->isChecked()) {
                return true;
...
void lec2_test2::onAnswerClicked() {
    // Проверяем ответы
...
bool lec2_test2::checkGroupAnswer(QGroupBox* group, const QVector<QuestionPair>& pairs, int pairIndex) {
...
void lec2_test2::setRadioButtonsEnabled(QGroupBox* group, bool enabled) {
    foreach(QObject* child, group->children()) {
        if (auto radio = qobject_cast<QRadioButton*>(child)) {
...
void lec2_test2::onNextLectureClicked() {
    // Создаем новое окно lec2_test3
    lec2_test3 *test3Window = new lec2_test3();

    // Настраиваем новое окно
    test3Window->setModal(true); // Если нужно модальное окно
