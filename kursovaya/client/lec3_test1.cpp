...
#include "ui_lec3_test1.h"
#include "lec3_test2.h"
#include <QRandomGenerator>
#include <QRadioButton>
#include <QMessageBox>

lec3_test1::lec3_test1(QWidget *parent)
    : QDialog(parent)
    , ui(new Ui::lec3_test1)
    , currentPair1(-1)
    , currentPair2(-1)
{
    ui->setupUi(this);
    ...
    // Подключаем сигналы
    connect(ui->agree, &QCheckBox::stateChanged, this, &lec3_test1::onAgreeClicked);
    connect(ui->pushButton, &QPushButton::clicked, this, &lec3_test1::onAnswerClicked);
    connect(ui->retry, &QCheckBox::stateChanged, this, &lec3_test1::onRetryClicked);
    connect(ui->pushButton_2, &QPushButton::clicked, this, &lec3_test1::onNextLectureClicked);
}

void lec3_test1::onRetryClicked(int state) {
    if (state == Qt::Checked) {
        resetTest();
    }
}

void lec3_test1::resetTest() {
...
void lec3_test1::resetRadioButtons(QGroupBox* group) {
...
void lec3_test1::initQuestionPairs() {
    ...
    // Группа 2 пары
    group2Pairs = {
        {ui->second_q, ui->second_ans, ui->sec_poyas, ui->Answer_D_2},
        {ui->fourth_q, ui->fourth_ans, ui->fourth_poyas, ui->Answer_D_2},
        {ui->six_q, ui->six_ans, ui->six_poyas, ui->Answer_B_2},
        {ui->eighth_q, ui->eighth_ans, ui->eight_poyas, ui->Answer_D_2}
    };
}

void lec3_test1::hideAllGroupElements(QGroupBox* group) {
    foreach(QObject* child, group->children()) {
...
void lec3_test1::onAgreeClicked(int state) {
    if (state == Qt::Checked) {
        startNewTest();
    }
}

void lec3_test1::startNewTest() {
    ui->scrollArea->hide();
...
void lec3_test1::showRandomQuestionPair(QGroupBox* group, const QVector<QuestionPair>& pairs) {
...
void lec3_test1::connectRadioButtons(QGroupBox* group) {
    foreach(QObject* child, group->children()) {
        if (auto radio = qobject_cast<QRadioButton*>(child)) {
            connect(radio, &QRadioButton::toggled, this, &lec3_test1::checkAnswers, Qt::UniqueConnection);
        }
    }
}

void lec3_test1::checkAnswers() {
...
bool lec3_test1::isGroupAnswered(QGroupBox* group) {
    foreach(QObject* child, group->children()) {
        if (auto radio = qobject_cast<QRadioButton*>(child)) {
            if (radio->isChecked()) {
                return true;
...
void lec3_test1::onAnswerClicked() {
    // Проверяем ответы
...
bool lec3_test1::checkGroupAnswer(QGroupBox* group, const QVector<QuestionPair>& pairs, int pairIndex) {
...
void lec3_test1::setRadioButtonsEnabled(QGroupBox* group, bool enabled) {
    foreach(QObject* child, group->children()) {
        if (auto radio = qobject_cast<QRadioButton*>(child)) {
...
void lec3_test1::onNextLectureClicked() {
    // Создаем новое окно lec3_test2
    lec3_test2 *test4Window = new lec3_test2();
