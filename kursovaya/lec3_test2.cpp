...
#include "ui_lec3_test2.h"
#include "lec3_test3.h"
#include <QRandomGenerator>
#include <QRadioButton>
#include <QMessageBox>

lec3_test2::lec3_test2(QWidget *parent)
    : QDialog(parent)
    , ui(new Ui::lec3_test2)
    , currentPair1(-1)
    , currentPair2(-1)
{
    ui->setupUi(this);
    ...
    // Подключаем сигналы
    connect(ui->agree, &QCheckBox::stateChanged, this, &lec3_test2::onAgreeClicked);
    connect(ui->pushButton, &QPushButton::clicked, this, &lec3_test2::onAnswerClicked);
    connect(ui->retry, &QCheckBox::stateChanged, this, &lec3_test2::onRetryClicked);
    connect(ui->pushButton_2, &QPushButton::clicked, this, &lec3_test2::onNextLectureClicked);
}

void lec3_test2::onRetryClicked(int state) {
    if (state == Qt::Checked) {
        resetTest();
    }
}

void lec3_test2::resetTest() {
...
void lec3_test2::resetRadioButtons(QGroupBox* group) {
...
void lec3_test2::initQuestionPairs() {
    ...
    group1Pairs = {
        ...
        {ui->seven_q, ui->seven_ans, ui->seven_poyas, ui->Answer_D},
        {ui->ninth_q, ui->ninth_ans, ui->ninth_poyas, ui->Answer_C}
    };

    // Группа 2 пары
    group2Pairs = {
        {ui->second_q, ui->second_ans, ui->sec_poyas, ui->Answer_C_2},
...
void lec3_test2::hideAllGroupElements(QGroupBox* group) {
    foreach(QObject* child, group->children()) {
...
void lec3_test2::onAgreeClicked(int state) {
    if (state == Qt::Checked) {
        startNewTest();
    }
}

void lec3_test2::startNewTest() {
    ui->scrollArea->hide();
...
void lec3_test2::showRandomQuestionPair(QGroupBox* group, const QVector<QuestionPair>& pairs) {
...
void lec3_test2::connectRadioButtons(QGroupBox* group) {
    foreach(QObject* child, group->children()) {
        if (auto radio = qobject_cast<QRadioButton*>(child)) {
            connect(radio, &QRadioButton::toggled, this, &lec3_test2::checkAnswers, Qt::UniqueConnection);
        }
    }
}

void lec3_test2::checkAnswers() {
...
bool lec3_test2::isGroupAnswered(QGroupBox* group) {
    foreach(QObject* child, group->children()) {
        if (auto radio = qobject_cast<QRadioButton*>(child)) {
            if (radio->isChecked()) {
                return true;
...
void lec3_test2::onAnswerClicked() {
    // Проверяем ответы
...
bool lec3_test2::checkGroupAnswer(QGroupBox* group, const QVector<QuestionPair>& pairs, int pairIndex) {
...
void lec3_test2::setRadioButtonsEnabled(QGroupBox* group, bool enabled) {
    foreach(QObject* child, group->children()) {
        if (auto radio = qobject_cast<QRadioButton*>(child)) {
...
void lec3_test2::onNextLectureClicked() {
    // Создаем новое окно lec3_test2
    lec3_test3 *test4Window = new lec3_test3();
