...
#include "ui_lec2_test1.h"
#include "lec2_test2.h"
#include <QRandomGenerator>
#include <QRadioButton>
#include <QMessageBox>

lec2_test1::lec2_test1(QWidget *parent)
    : QDialog(parent)
    , ui(new Ui::lec2_test1)
    , currentPair1(-1)
    , currentPair2(-1)
{
    ui->setupUi(this);
    ...
    // Подключаем сигналы
    connect(ui->agree, &QCheckBox::stateChanged, this, &lec2_test1::onAgreeClicked);
    connect(ui->pushButton, &QPushButton::clicked, this, &lec2_test1::onAnswerClicked);
    connect(ui->retry, &QCheckBox::stateChanged, this, &lec2_test1::onRetryClicked);
    connect(ui->pushButton_2, &QPushButton::clicked, this, &lec2_test1::onNextLectureClicked);
}

void lec2_test1::onRetryClicked(int state) {
    if (state == Qt::Checked) {
        resetTest();
    }
}

void lec2_test1::resetTest() {
...
void lec2_test1::resetRadioButtons(QGroupBox* group) {
...
void lec2_test1::initQuestionPairs() {
    ...
    group2Pairs = {
        ...
        {ui->six_q, ui->six_ans, ui->six_poyas, ui->Answer_A_2},
        {ui->eighth_q, ui->eighth_ans, ui->eight_poyas, ui->Answer_A_2}
    };
}

void lec2_test1::hideAllGroupElements(QGroupBox* group) {
    foreach(QObject* child, group->children()) {
...
void lec2_test1::onAgreeClicked(int state) {
    if (state == Qt::Checked) {
        startNewTest();
    }
}

void lec2_test1::startNewTest() {
    ui->scrollArea->hide();
...
void lec2_test1::showRandomQuestionPair(QGroupBox* group, const QVector<QuestionPair>& pairs) {
...
void lec2_test1::connectRadioButtons(QGroupBox* group) {
    foreach(QObject* child, group->children()) {
        if (auto radio = qobject_cast<QRadioButton*>(child)) {
            connect(radio, &QRadioButton::toggled, this, &lec2_test1::checkAnswers, Qt::UniqueConnection);
        }
    }
}

void lec2_test1::checkAnswers() {
...
bool lec2_test1::isGroupAnswered(QGroupBox* group) {
    foreach(QObject* child, group->children()) {
        if (auto radio = qobject_cast<QRadioButton*>(child)) {
            if (radio->isChecked()) {
                return true;
...
void lec2_test1::onAnswerClicked() {
    // Проверяем ответы
...
bool lec2_test1::checkGroupAnswer(QGroupBox* group, const QVector<QuestionPair>& pairs, int pairIndex) {
...
void lec2_test1::setRadioButtonsEnabled(QGroupBox* group, bool enabled) {
    foreach(QObject* child, group->children()) {
        if (auto radio = qobject_cast<QRadioButton*>(child)) {
...
void lec2_test1::onNextLectureClicked() {
    // Создаем новое окно lec1_test2
    lec2_test2 *test2Window = new lec2_test2();
