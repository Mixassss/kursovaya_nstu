...
#include "ui_lec1_test2.h"
#include <QRandomGenerator>
#include <QRadioButton>
#include <QMessageBox>

lec1_test2::lec1_test2(QWidget *parent)
    : QDialog(parent)
    , ui(new Ui::lec1_test2)
    , currentPair1(-1)
    , currentPair2(-1)
{
    ui->setupUi(this);
    ...
    // Подключаем сигналы
    connect(ui->agree, &QCheckBox::stateChanged, this, &lec1_test2::onAgreeClicked);
    connect(ui->pushButton, &QPushButton::clicked, this, &lec1_test2::onAnswerClicked);
    connect(ui->retry, &QCheckBox::stateChanged, this, &lec1_test2::onRetryClicked);
    connect(ui->pushButton_2, &QPushButton::clicked, this, &lec1_test2::onNextLectureClicked);
}

void lec1_test2::onRetryClicked(int state) {
    if (state == Qt::Checked) {
        resetTest();
    }
}

void lec1_test2::resetTest() {
...
void lec1_test2::resetRadioButtons(QGroupBox* group) {
...
void lec1_test2::initQuestionPairs() {
...
void lec1_test2::hideAllGroupElements(QGroupBox* group) {
    foreach(QObject* child, group->children()) {
...
void lec1_test2::onAgreeClicked(int state) {
    if (state == Qt::Checked) {
        startNewTest();
    }
}

void lec1_test2::startNewTest() {
    ui->scrollArea->hide();
...
void lec1_test2::showRandomQuestionPair(QGroupBox* group, const QVector<QuestionPair>& pairs) {
...
void lec1_test2::connectRadioButtons(QGroupBox* group) {
    foreach(QObject* child, group->children()) {
        if (auto radio = qobject_cast<QRadioButton*>(child)) {
            connect(radio, &QRadioButton::toggled, this, &lec1_test2::checkAnswers, Qt::UniqueConnection);
        }
    }
}

void lec1_test2::checkAnswers() {
...
bool lec1_test2::isGroupAnswered(QGroupBox* group) {
    foreach(QObject* child, group->children()) {
        if (auto radio = qobject_cast<QRadioButton*>(child)) {
            if (radio->isChecked()) {
                return true;
...
void lec1_test2::onAnswerClicked() {
    // Проверяем ответы
...
bool lec1_test2::checkGroupAnswer(QGroupBox* group, const QVector<QuestionPair>& pairs, int pairIndex) {
...
void lec1_test2::setRadioButtonsEnabled(QGroupBox* group, bool enabled) {
    foreach(QObject* child, group->children()) {
        if (auto radio = qobject_cast<QRadioButton*>(child)) {
...
void lec1_test2::onNextLectureClicked() {
    // Создаем новое окно lec1_test2
    lec1_test3 *test3Window = new lec1_test3();
