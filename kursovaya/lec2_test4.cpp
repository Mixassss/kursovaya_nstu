...
#include "ui_lec2_test4.h"
#include "test2.h"
#include <QRandomGenerator>
#include <QRadioButton>
#include <QMessageBox>

lec2_test4::lec2_test4(QWidget *parent)
    : QDialog(parent)
    , ui(new Ui::lec2_test4)
    , currentPair1(-1)
    , currentPair2(-1)
{
    ui->setupUi(this);
    ...
    // Подключаем сигналы
    connect(ui->agree, &QCheckBox::stateChanged, this, &lec2_test4::onAgreeClicked);
    connect(ui->pushButton, &QPushButton::clicked, this, &lec2_test4::onAnswerClicked);
    connect(ui->retry, &QCheckBox::stateChanged, this, &lec2_test4::onRetryClicked);
    connect(ui->pushButton_2, &QPushButton::clicked, this, &lec2_test4::onNextLectureClicked);
}

void lec2_test4::onRetryClicked(int state) {
    if (state == Qt::Checked) {
        resetTest();
    }
}

void lec2_test4::resetTest() {
...
void lec2_test4::resetRadioButtons(QGroupBox* group) {
...
void lec2_test4::initQuestionPairs() {
    ...
    group1Pairs = {
        ...
        {ui->ninth_q, ui->ninth_ans, ui->ninth_poyas, ui->Answer_C}
    };

    // Группа 2 пары
    group2Pairs = {
        {ui->second_q, ui->second_ans, ui->sec_poyas, ui->Answer_B_2},
        {ui->fourth_q, ui->fourth_ans, ui->fourth_poyas, ui->Answer_B_2},
        {ui->six_q, ui->six_ans, ui->six_poyas, ui->Answer_B_2},
        {ui->eighth_q, ui->eighth_ans, ui->eight_poyas, ui->Answer_A_2}
    };
}

void lec2_test4::hideAllGroupElements(QGroupBox* group) {
    foreach(QObject* child, group->children()) {
...
void lec2_test4::onAgreeClicked(int state) {
    if (state == Qt::Checked) {
        startNewTest();
    }
}

void lec2_test4::startNewTest() {
    ui->scrollArea->hide();
...
void lec2_test4::showRandomQuestionPair(QGroupBox* group, const QVector<QuestionPair>& pairs) {
...
void lec2_test4::connectRadioButtons(QGroupBox* group) {
    foreach(QObject* child, group->children()) {
        if (auto radio = qobject_cast<QRadioButton*>(child)) {
            connect(radio, &QRadioButton::toggled, this, &lec2_test4::checkAnswers, Qt::UniqueConnection);
        }
    }
}

void lec2_test4::checkAnswers() {
...
bool lec2_test4::isGroupAnswered(QGroupBox* group) {
    foreach(QObject* child, group->children()) {
        if (auto radio = qobject_cast<QRadioButton*>(child)) {
            if (radio->isChecked()) {
                return true;
...
void lec2_test4::onAnswerClicked() {
    // Проверяем ответы
...
bool lec2_test4::checkGroupAnswer(QGroupBox* group, const QVector<QuestionPair>& pairs, int pairIndex) {
...
void lec2_test4::setRadioButtonsEnabled(QGroupBox* group, bool enabled) {
    foreach(QObject* child, group->children()) {
        if (auto radio = qobject_cast<QRadioButton*>(child)) {
...
void lec2_test4::onNextLectureClicked() {
    // Закрываем текущее окно (lec2_test4)
    this->close();

    // Находим и закрываем окно lec2_test4
    QWidgetList widgets = QApplication::topLevelWidgets();
    for (QWidget* widget : widgets) {
        if (widget->inherits("test2")) {
            widget->close();
            break;
        }
    }
